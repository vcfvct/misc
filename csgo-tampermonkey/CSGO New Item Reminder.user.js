...
// Your code here...
	...
	GM_addStyle('#info-banner { top: 0; height: 40px; width: 100%;  background-color: yellow; color: black; border-radius: 6px; font-size: 2em!important; text-align:center;}');

	let pageTitle = $(document).find("title").text();
	let itemNumber = parseInt($('#searchResults_total').text());
	...
	if (isNaN(previousItemNumber)) {
	    ...
	    if (refreshing && itemNumber > previousItemNumber) {
	        ...
	        var notificationDetails = {
	            text: msg,
	            title: 'Refresh Stopped, you can manual refresh again by pressing the button.',
	            timeout: 10000,
	            onclick: function() {
